...
def list_files(conn):
    '''
    List all files in the server directory and send to client
    '''
    try:
        files = os.listdir('.')
        for f in files:
        ...
        print(e)
        error_message = "There has been an error listing the files in the directory."
        conn.sendall(error_message.encode('utf-8'))
    ...
    while (True):
        ...
        if command in command_list:
            ...
            if command == "LIST":
                ...
                file_list_string = '\n'.join(file_list)
                conn.sendall(file_list_string.encode('utf-8'))
