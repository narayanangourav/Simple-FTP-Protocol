...
def list(sock):
    '''
    List all the files in the server's directory
    '''
    try:
        # send the LIST command to the server
        sock.sendall("LIST".encode('utf-8'))
        # receive the file list from the server and print it
        data = sock.recv(1024).decode("utf-8")
        print(data)
    except Exception as e:
        print(e)
        error_message = "There has been an error listing the files on the server."
        sock.sendall(error_message.encode('utf-8'))


command_list = ["QUIT","CLOSE", "OPEN", "GET","PUT","LIST"]
...
while (True):
    ...
    if command in command_list:
        ...
        if command == "LIST":
            ...
            # Send the server a message to request the list of files
            sock.sendall("LIST".encode("utf-8"))

            # Receive the list of files and print it
            data = sock.recv(1024).decode("utf-8")
            print(data)
